...
| **P4 历史与限流** | 历史记录服务 + Redis 限流 | history 路由 + 限流中间件 | 分页查询正确；超限返回 429 | 1~1.5 天 |
...
> 当前状态：**P0 框架、P1 数据层、P2 认证均已验收通过**（2026-09-08：
> P2 curl 全流程 11 项 + 认证 8 测试全绿；P1 迁移回滚/重放验证通过）。
> P3 AI 能力是下一步（需先在 .env 配置真实 LLM_API_KEY），P4~P7 按里程碑推进。
