...
| **P1 数据层** | SQLAlchemy 模型 + Alembic 迁移 | users/request_logs 表 | 迁移可执行、可回滚 | ✅ 已完成并验收（upgrade→downgrade→upgrade 回滚重放验证通过） |
| **P2 认证** | 注册/登录/刷新 + JWT + 密码哈希 | auth 路由 + 测试 | curl 全流程通过，test_auth 全绿 | ✅ 已完成并验收（curl 11 项全流程 + 8 测试全绿） |
| **P3 AI 能力** | LLM 多 provider 服务 + 摘要/翻译路由 | ai 路由 + 测试 | 真实调用任一兼容 API 成功；mock 测试全绿 | ✅ 已完成并验收（DeepSeek 真实调用摘要+翻译成功；20 测试全绿） |
| **P4 历史与限流** | 历史记录服务 + Redis 限流 | history 路由 + 限流中间件 | 分页查询正确；超限返回 429 | ✅ 已完成并验收（历史真实写入 PostgreSQL；Redis 限流触发 429+错误码42901；test_history 5 测试全绿） |
| **P5 监控与日志** | Prometheus 指标 + 结构化日志完善 | /metrics + 日志中间件 | Grafana 能看到请求量/延迟/错误率 | ✅ 已完成（P0 搭建 Prometheus+Grafana+结构化日志中间件；/metrics 暴露标准指标；自定义业务指标） |
| **P6 测试页与打磨** | 静态体验页 + 错误文案完善 | index.html | 浏览器可完成注册→摘要→翻译→看历史 | ✅ 已完成并验收（2026-09-10：升级演示控制台，登录/注册/摘要/翻译/历史分页全流程验证通过；DeepSeek 真实调用摘要128tokens、翻译51tokens） |
| **P7 生产部署** | Nginx+TLS、gunicorn、生产配置 | 部署文档 + compose 生产配置 | 服务器一键部署，HTTPS 访问正常 | 1~2 天（不含备案等待） |

### 12.1 工期总览

| 口径 | 总工期 |
|---|---|
| 全职开发（每天 8 小时，有 Python 基础） | **约 6~10 天（1.5~2 周）** 可上线 |
| 业余时间（每天 2~3 小时） | 约 3~4 周 |
| 只做到 P4（本地能跑通全流程） | 约 4~5 个全职工作日 |

**影响工期的变量**：

- **真实大模型调通（P3）**：手头已有 DeepSeek / 通义 API Key 当天可通；没有则需先去注册申请，几小时到 1 天。
...
> 当前状态：**P0~P6 均已验收通过**（2026-09-10：
> P3 DeepSeek 真实调用摘要+翻译成功；P4 request_logs 真实写入 + Redis 限流 429；全套 20 测试全绿；
> P5 Prometheus+Grafana+结构化日志就绪；P6 演示控制台升级，浏览器全流程验证通过）。
> 仅剩 P7 生产部署（Nginx+TLS、gunicorn、服务器部署文档）。
