...
| **P5 监控与日志** | Prometheus 指标 + 结构化日志完善 | /metrics + 日志中间件 | Grafana 能看到请求量/延迟/错误率 | 1 天 |
| **P6 测试页与打磨** | 静态体验页 + 错误文案完善 | index.html | 浏览器可完成注册→摘要→翻译→看历史 | 0.5~1 天 |
...
> 当前状态：**P0 框架、P1 数据层、P2 认证、P3 AI 能力、P4 历史与限流均已验收通过**（2026-09-09：
> P3 DeepSeek 真实调用摘要+翻译成功；P4 request_logs 真实写入 + Redis 限流 429；全套 20 测试全绿）。
> P5 监控与日志、P6 测试页打磨、P7 生产部署按里程碑推进。
